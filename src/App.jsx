...
// Custom Discord Icon - Optimized with pointer-events fix
const DiscordIcon = ({ className }) => (
  <svg 
    viewBox="0 0 24 24" 
    fill="currentColor" 
    className={className}
    style={{ pointerEvents: 'none' }} // Prevents SVG from blocking click events
...
const PROJECTS = [
  ...
  {
    title: "Blender Model #3",
    category: "3D Modeling",
    description: "Environment prop set featuring modular architecture and custom texturing.",
    tech: ["Blender", "Texturing"],
    link: "#",
    github: "#",
  ...
  {
    title: "Blender Model #4",
    category: "3D Modeling",
    description: "Weapon asset pack designed for real-time performance and stylized visuals.",
    tech: ["Blender", "Optimization"],
    link: "#",
    github: "#",
...
const SKILLS = [
  ...
  { name: "Bot Dev", icon: <DiscordIcon className="w-5 h-5" />, items: ["Node.js", "discord.js", "MongoDB", "API"] },
    ...
    <div className="flex flex-col min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-emerald-500/30 selection:text-emerald-200 overflow-x-hidden relative w-full">
          ...
          <div className="hidden md:flex items-center gap-8">
            {['home', 'about', 'skills', 'projects'].map((item) => (
              <button
                ...
                className={`text-xs font-black tracking-widest transition-all hover:text-emerald-400 uppercase relative ${activeSection === item ? 'text-emerald-400' : 'text-slate-500'}`}
              >
                {item}
                {activeSection === item && <span className="absolute -bottom-1 left-0 w-full h-0.5 bg-emerald-500 rounded-full" />}
              </button>
            ))}
            <button onClick={() => handleNavClick('contact')} className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-lg shadow-emerald-900/20 active:scale-95">
              Contact
            </button>
          </div>
      ...
      <div className={`fixed inset-0 bg-slate-950 z-[100] transition-transform duration-500 md:hidden flex items-center justify-center ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex flex-col items-center gap-10">
           {['home', 'about', 'skills', 'projects', 'contact'].map((item) => (
            <button key={item} onClick={() => handleNavClick(item)} className={`text-4xl font-black uppercase tracking-tighter transition-all ${activeSection === item ? 'text-emerald-400' : 'text-slate-700'}`}>
              {item}
            </button>
          ))}
        </div>
      ...
      <main className="flex-grow relative z-10 w-full">
        ...
        <section id="home" className="px-6 min-h-screen flex flex-col items-center justify-center text-center overflow-hidden relative w-full">
          {/* Blobs with pointer-events-none to prevent click blockage */}
          ...
          <div className="max-w-5xl mx-auto z-10 reveal-up">
            <span className="px-5 py-2 rounded-2xl border border-emerald-500/30 bg-emerald-500/5 text-emerald-400 text-[10px] font-black uppercase tracking-[0.3em] mb-8 inline-block shadow-inner">
              <Sparkles className="w-3 h-3 inline mr-2 mb-0.5" /> Worlds & Systems
            </span>
            <h1 className="text-6xl md:text-[110px] font-black mb-10 tracking-tighter leading-[0.85] uppercase">
              Beyond <br /> <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-emerald-400 to-indigo-400">Interactive.</span>
            </h1>
            <p className="text-slate-400 text-lg md:text-2xl max-w-3xl mx-auto mb-14 leading-relaxed font-medium">
              {PERSONAL_INFO.about}
            </p>
            <div className="flex flex-col sm:flex-row gap-6 justify-center">
              <button onClick={() => handleNavClick('projects')} className="px-12 py-5 bg-emerald-600 hover:bg-emerald-500 rounded-2xl font-black text-lg flex items-center justify-center gap-3 transition-all hover:scale-105 active:scale-95 shadow-2xl shadow-emerald-900/40 group">
                Browse Projects <ChevronRight className="w-6 h-6 group-hover:translate-x-1 transition-transform" />
              </button>
              <button onClick={() => handleNavClick('contact')} className="px-12 py-5 bg-slate-900/50 backdrop-blur-sm hover:bg-slate-800 rounded-2xl font-black text-lg transition-all border border-slate-800 hover:border-slate-700 shadow-xl">
                Let's Chat
              </button>
            </div>
          </div>
          
          <div className="absolute bottom-10 animate-bounce text-slate-600 opacity-50">
          ...
          <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-20 items-center overflow-hidden">
            <div className="relative group perspective-1000 reveal-left">
              ...
              <div className="relative rounded-[40px] bg-slate-900 border border-slate-800 aspect-[4/5] flex flex-col items-center justify-center overflow-hidden shadow-2xl transition-all duration-500 group-hover:scale-[1.02]">
                <Box className="w-32 h-32 text-slate-800 mb-6 group-hover:text-emerald-500/20 transition-all duration-700" />
                <p className="text-slate-600 font-black tracking-[0.4em] uppercase text-[10px]">Technical Artist</p>
            ...
            <div className="reveal-right">
              <h2 className="text-5xl font-black mb-10 flex items-center gap-5 tracking-tighter">
                <div className="w-12 h-1 bg-emerald-500 rounded-full" /> WHO I AM
              </h2>
              <div className="space-y-8 text-slate-400 text-xl leading-relaxed font-medium">
                {PERSONAL_INFO.detailedAbout.map((p, i) => <p key={i}>{p}</p>)}
              </div>
              <div className="mt-12 flex gap-5">
                 <a href={PERSONAL_INFO.github} target="_blank" rel="noopener noreferrer" className="p-5 bg-slate-900 border border-slate-800 rounded-2xl text-slate-400 hover:text-emerald-400 transition-all transform hover:scale-110"><Github className="w-8 h-8" /></a>
                 {/* Re-integrated Discord Link with profile redirect */}
                 <a href={PERSONAL_INFO.discordLink} target="_blank" rel="noopener noreferrer" className="p-5 bg-slate-900 border border-slate-800 rounded-2xl text-slate-400 hover:text-emerald-400 transition-all transform hover:scale-110 flex items-center justify-center">
              ...
              {SKILLS.map((skill, idx) => (
                <div key={idx} className="p-10 bg-slate-900/50 border border-slate-800 rounded-[3rem] hover:border-emerald-500/40 transition-all group reveal-up" style={{ transitionDelay: `${idx * 100}ms` }}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {skill.items.map((item, i) => <span key={i} className="px-4 py-2 bg-slate-800 text-slate-400 text-[10px] font-black uppercase tracking-widest rounded-xl">{item}</span>)}
                  </div>
                </div>
              ))}
            </div>
              ...
              {filteredProjects.map((project, idx) => (
                <div key={project.title} className="group bg-slate-900/40 border border-slate-800 rounded-[2.5rem] overflow-hidden hover:translate-y-[-12px] transition-all duration-500 reveal-up">
                  ...
                  <div className="p-8">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {project.tech.map((t, i) => <span key={i} className="text-[9px] font-black text-slate-500 bg-slate-950 px-2 py-1 rounded-lg border border-slate-800/50">{t}</span>)}
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="relative z-10 grid lg:grid-cols-2 gap-20">
              <div className="reveal-left">
                ...
                <div className="space-y-10">
                  <div className="flex items-center gap-6 group">
                    ...
                    <div>
                      <p className="text-xs text-slate-500 font-black uppercase tracking-widest">Email</p>
                      <p className="text-slate-200 font-bold text-xl">{PERSONAL_INFO.email}</p>
                    </div>
                  </div>
                  {/* Robust Discord hit-zone in Contact section */}
                  <a href={PERSONAL_INFO.discordLink} target="_blank" rel="noopener noreferrer" className="flex items-center gap-6 group hover:translate-x-2 transition-transform cursor-pointer">
                    <DiscordIcon className="w-8 h-8 text-blue-400 group-hover:text-blue-300 transition-colors" />
                    <div>
                      <p className="text-xs text-slate-500 font-black uppercase tracking-widest">Discord</p>
                      <p className="text-slate-200 font-bold text-xl hover:text-emerald-400 transition-colors">{PERSONAL_INFO.discord}</p>
                    </div>
                  </a>
                </div>
              </div>
              <form onSubmit={handleContactSubmit} className="space-y-6 reveal-right">
                <input 
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-8 py-5 focus:ring-2 focus:ring-emerald-500/20 transition-all outline-none" 
                  placeholder="Name" 
                />
                <input 
                  type="email" 
                ...
                <textarea 
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-8 py-5 focus:ring-2 focus:ring-emerald-500/20 transition-all outline-none resize-none" 
                  placeholder="Message"
                ></textarea>
                <button 
                  type="submit" 
                  disabled={formStatus === 'sending'} 
                  className="w-full py-6 bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-800 rounded-2xl font-black text-xl flex items-center justify-center gap-4 transition-all"
                >
                  {formStatus === 'sending' ? 'Sending...' : <>Send Transmission <Terminal className="w-6 h-6" /></>}
                </button>
                {formStatus === 'success' && <div className="p-4 bg-emerald-500/10 text-emerald-400 text-center font-black rounded-xl animate-pulse mt-4">Transmission Received! I will get back to you shortly.</div>}
                {formStatus === 'error' && <div className="p-4 bg-red-500/10 text-red-400 text-center font-black rounded-xl mt-4">Error: Transmission failed. Please try again or email me directly.</div>}
              </form>
            </div>
          </div>
        </section>
      ...
      <footer className="py-24 px-6 border-t border-slate-900/50 text-center bg-slate-950 relative z-20 mt-auto w-full overflow-hidden">
        ...
        <div className="flex justify-center gap-12 mb-12">
           <a href={PERSONAL_INFO.github} target="_blank" rel="noopener noreferrer" className="text-slate-600 hover:text-emerald-400 transition-all transform hover:scale-125"><Github className="w-8 h-8" /></a>
           <a href={PERSONAL_INFO.discordLink} target="_blank" rel="noopener noreferrer" className="text-slate-600 hover:text-emerald-400 transition-all transform hover:scale-125 flex items-center justify-center">
              <DiscordIcon className="w-8 h-8" />
           </a>
        </div>
